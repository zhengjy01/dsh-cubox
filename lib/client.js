	...
	factory: (require) => {
		...
		*/
		...
		function statusText(view) {
			...
			return "已配置 · 服务器 " + view.server + " · token " + view.tokenMasked + " · 定时同步每 " + view.syncMinutes + " 分钟 · 最近同步 " + (view.lastSyncAt !== "" ? view.lastSyncAt : "从未") + " · 缓存卡片 " + view.cachedCards + " 条 / 标注 " + view.cachedAnnotations + " 条 · 导出目录 " + (view.outputDir !== "" ? view.outputDir : "未设置");
		}
		...
		function CuboxSettingsPanel() {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						style: s.hint,
						children: "API 扩展链接获取：Cubox 偏好设置 → 扩展中心和自动化 → API 扩展 → 启用并复制链接。链接是个人身份凭证，请勿泄露。 token 与同步快照分别存于 ~/.dsh/dsh-cubox.json 与 ~/.dsh/dsh-cubox-cache.json（权限 0600）。 配置导出目录后，每次同步会把今日收藏写成 Markdown（每卡片一个文件 + 每日总结大纲）到该目录。 同步后也可用 cubox_today（今日总结大纲）与 cubox_annotations（笔记标注汇总）等工具。"
					})
				]
			});
		}
