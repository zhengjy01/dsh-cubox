	...
	factory: (require) => {
		...
		*/
		...
		function statusText(view) {
			...
			return "已配置 · 服务器 " + view.server + " · token " + view.tokenMasked + " · 定时同步每 " + view.syncMinutes + " 分钟 · 最近同步 " + (view.lastSyncAt !== "" ? view.lastSyncAt : "从未") + " · 缓存卡片 " + view.cachedCards + " 条 / 标注 " + view.cachedAnnotations + " 条";
		}
		...
		function CuboxSettingsPanel() {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						style: s.hint,
						children: "API 扩展链接获取：Cubox 偏好设置 → 扩展中心和自动化 → API 扩展 → 启用并复制链接。链接是个人身份凭证，请勿泄露。 token 与同步快照分别存于 ~/.dsh/dsh-cubox.json 与 ~/.dsh/dsh-cubox-cache.json（权限 0600）。 同步后可用 cubox_today（今日总结大纲）与 cubox_annotations（笔记标注汇总）等工具。"
					})
				]
			});
		}
