...
async function doSync(api, store, opts = {}) {
	...
	return {
		ok: true,
		message: "同步完成：拉取卡片 " + cards.length + " 条、标注 " + annotations.length + " 条；缓存现有卡片 " + mergedCards.length + " 条、标注 " + mergedAnnotations.length + " 条。" + (exportedFiles > 0 ? "已导出 " + exportedFiles + " 个 Markdown 文件到 " + outputDir.trim() : "") + (warnings.length > 0 ? "\n警告：" + warnings.join("；") : ""),
