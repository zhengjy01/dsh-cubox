...
var CuboxApi = class {
	...
	async request(input, cfg, init) {
		...
		const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS$1);
		try {
...
async function doSync(api, store, opts = {}) {
	...
	return {
		ok: true,
		message: "同步完成：拉取卡片 " + cards.length + " 条、标注 " + annotations.length + " 条；缓存现有卡片 " + mergedCards.length + " 条、标注 " + mergedAnnotations.length + " 条。" + (exportedFiles > 0 ? "已导出 " + exportedFiles + " 个收藏 Markdown 到 " + outputDir.trim() : "") + (briefPath !== "" ? "已生成简报：" + briefPath : "") + (warnings.length > 0 ? "\n警告：" + warnings.join("；") : ""),
...
function formatCollectionForPrompt(cache, date) {
	const isToday = (iso) => {
		const d = new Date(iso);
		return !Number.isNaN(d.getTime()) && d.getFullYear() === date.getFullYear() && d.getMonth() === date.getMonth() && d.getDate() === date.getDate();
	};
...
async function writeDailyBrief(cache, outputDir, llm) {
	const now = /* @__PURE__ */ new Date();
...
//#endregion
export { CUBOX_API, CUBOX_GUIDANCE, CuboxApi, CuboxApiError, CuboxStore, DEFAULT_LLM_PROMPT, apply, buildTools, cachePath, chatComplete, configPath, cuboxCardsTool, cuboxConfigTool, cuboxStatusTool, cuboxSyncTool, defineTool, doSync, exportSyncToMarkdown, formatApiTime, formatCollectionForPrompt, inject, llmConfigured, makeRoutes, mask, name, parseApiLink, readCache, todayRange, writeCache, writeDailyBrief };
